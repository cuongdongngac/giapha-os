...
interface DashboardPostsViewStandaloneProps {
  isAdmin?: boolean;
  initialPosts?: Post[];
  initialPostsCount?: number;
}

export default function DashboardPostsViewStandalone({ isAdmin, initialPosts, initialPostsCount }: DashboardPostsViewStandaloneProps) {
  const { 
  ...
  if (isCreatingPost) {
    return (
      <div className="max-w-5xl mx-auto px-4 py-8 w-full">
        <PostForm onSuccess={() => { handleBack(); refresh(); }} onCancel={handleBack} />
      </div>
    );
  }
  ...
  if (editingPostId) {
    ...
    return (
      <div className="max-w-5xl mx-auto px-4 py-8 w-full">
        <PostForm initialData={editingPost} isEditing={true} onSuccess={() => { handleBack(); refresh(); }} onCancel={handleBack} />
      </div>
    );
  }
      ...
      {totalCount > 10 && (
        <div className="flex items-center justify-center gap-4 pt-8 border-t border-stone-100">
          ...
          <div className="flex items-center gap-1">
            {Array.from({ length: Math.ceil(totalCount / 10) }, (_, i) => i + 1).map((pageNumber: number) => (
              <button
                key={pageNumber}
                onClick={() => setPage(pageNumber)}
                className={`size-8 rounded-lg flex items-center justify-center text-sm font-bold transition-all ${
                  page === pageNumber ? 'bg-amber-100 text-amber-800' : 'text-stone-400 hover:bg-stone-50'
                }`}
              >
                {pageNumber}
              </button>
