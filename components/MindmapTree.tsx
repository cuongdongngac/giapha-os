...
const MindmapNode = memo(
  ({
    ...
    return (
      <div
        className={`relative py-1.5 ${level > 0 ? "pl-6" : "pl-0"} overflow-visible`}
      >
        ...
        <div className="flex items-center gap-2 group relative z-10 overflow-visible">
          ...
          <div className="size-5 flex items-center justify-center shrink-0 z-10 bg-transparent">
            {hasChildren ? (
              <button
                ...
                className="size-5 flex items-center justify-center bg-white hover:bg-amber-50 border border-stone-200 rounded shadow-sm text-stone-500 hover:text-amber-600 focus:outline-none transition-colors relative z-20"
          ...
          {(() => {
            return (
              <div className="flex flex-col gap-1 overflow-visible">
                <MindmapNodeCard
                  person={data.person}
                  showAvatar={ctx.showAvatar}
                  onShowInfo={() => ctx.setMemberModalId(data.person.id)}
                  onSetRoot={() => ctx.handleRootChange(data.person.id)}
